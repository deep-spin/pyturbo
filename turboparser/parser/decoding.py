...
def decode_marginals(parts: DependencyParts, scores: dict) -> tuple:
    """
...
def generate_arc_mask(parts, scores, max_heads, threshold=None):
    ...
    marginals, _, entropy = decode_marginals(parts, scores)
    arc_marginals = marginals[:parts.num_arcs]
    candidate_heads = defaultdict(list)
    new_mask = np.zeros_like(parts.arc_mask)

    for arc, prob in zip(arcs, arc_marginals):
        h, m = arc
